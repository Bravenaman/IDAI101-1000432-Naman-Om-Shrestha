# IDAI102-1000432-Naman-Om-Shrestha
...
| `find_academic_building`| Provides the location of a specific academic building. | "Where is the Science Block?", "Find the Engineering Hall"  |
| `find_event` | Finds the location or time for a campus event. | "Where's today's seminar happening?", "What's on this weekend?"  |
| `find_hours` | Provides the operating hours for a campus service or facility. | "What are the library hours?", "When does the admin office open?" |
| `find_lab` | Gives directions to a specific computer or science lab. | "I need directions to the Computer Science lab", "Where is the physics lab?"  |
...
| `find_service_building` | Locates a specific student or campus service building. | "Where is the admin office?", "How to get to the library?"  |
...
| `get_emergency_contact`| Provides critical emergency contact information. | "I need help", "Emergency on campus", "Campus security number"  |
| `get_nearby_recreation` | Recommends nearby recreational activities or places. | "What is there to do around here?", "Nearby parks or cafes"  |
| `get_policy` | Explains a specific campus policy or rule. | "What is the attendance policy?", "Tell me the hostel rules"  |
| `get_transportation` | Gives information on transportation options. | "Campus bus schedule", "Where is the nearest metro?"  |
